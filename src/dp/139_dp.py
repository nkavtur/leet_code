...
class Solution:
    def wordBreak(self, s: str, words: List[str]) -> bool:
        ...
        return self.word_break_recursively(trie, 0, len(s), memo)

    def word_break_recursively(self, trie, start, max_end, memo):
        if start == max_end:
            return True

        for end in trie[start]:

            if (end + 1) in memo:
                continue

            memo[end + 1] = self.word_break_recursively(trie, end + 1, max_end, memo)
